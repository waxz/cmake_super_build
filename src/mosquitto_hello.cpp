...
void on_connect(struct mosquitto *mosq, void *obj, int reason_code)
{
    ...
    rc = mosquitto_subscribe(mosq, NULL, "hello/123", 1);
...
void publish_sensor_data(struct mosquitto *mosq)
{
     ...
     * topic = "hello/123" - the topic on which this message will be published
     * payloadlen = strlen(payload) - the length of our payload in bytes
     * payload - the actual payload
     * qos = 2 - publish with QoS 2 for this example
     * retain = false - do not use the retained message feature for this message
     */
    rc = mosquitto_publish(mosq, NULL, "hello/123", strlen(payload), payload, 2, false);
    if(rc != MOSQ_ERR_SUCCESS){
...
int main(int argc, char *argv[])
{
    ...
    rc = mosquitto_connect(mosq, "10.23.0.125", 1883, 60);
    if(rc != MOSQ_ERR_SUCCESS){
